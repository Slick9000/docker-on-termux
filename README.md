...
this will make sure all packages are up to date.
...
>  qemu-img create -f qcow2 hdd.img 16G (16G can be altered to any storage size you want, in gigabytes)
...
>  setup-alpine to perform the regular alpine setup. (personally, i usually set the username and password both as root, set the hdd.img as the sys storage partition, and leave everything else as default).
...
to do so, install vim by running:

>  apk add --update vim (vim is a text editor)
...
>  rc-update add docker default (this will make it run automatically on all future startups of alpine)

to begin it without restarting, run:
